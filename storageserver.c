...
int is_sentence_delimiter(char c) {
    for (int i = 0; i < MAX_DELIMITERS; i++) {
        if (c == SENTENCE_DELIMITERS[i]) return 1;
    }
    return 0;
}
...
int has_delimiter_ending(const char* str) {
    ...
    for (int i = len - 1; i >= 0; i--) {
        if (is_sentence_delimiter(str[i])) return 1;
...
static int ensure_lock_capacity(FileData *file, int required) {
    ...
    if (file->locks_capacity >= required && file->sentence_locks != NULL) return 0;
...
int init_sentence_locks(FileData *file) {
    ...
    if (ensure_lock_capacity(file, num_sentences + 1) != 0) return -1;
...
void unlock_sentence(FileData *file, int sent_num, const char *username) {
    if (sent_num < 0 || sent_num >= file->locks_capacity) return;
...
int save_file(FileData *file) {
    ...
    if (fflush(fp) != 0) {
        ...
        fclose(fp);
        unlink(temp_path);
        if (access(backup_path, F_OK) == 0) rename(backup_path, filepath);
        return -1;
    }
...
static void process_client(int client_sock) {
    ...
    case MSG_WRITE_FILE: {
            ...
            while (lock_retries < 3) {
                ...
                if (lock_result == ERR_SUCCESS) break;
            ...
            if (cur_len > 0) memcpy(current_sentence, content + sent_start, cur_len);
            ...
            while (seg) {
                // Trim leading spaces
                while (*seg == ' ' || *seg == '\t') seg++;
                if (*seg == '\0') { seg = strtok_r(NULL, ";", &seg_save); continue; }
                // Parse index
                char *p = seg;
                char *endnum = NULL;
                ...
                while (endnum && (*endnum == ' ' || *endnum == '\t')) endnum++;
                const char *ins_content = endnum && *endnum ? endnum : "";

                // Determine insertion position
                int pos;
                if (num_words == 0) {
                    if (k == 0 || k == 1) pos = 0; else {
                        free(updates_dup);
                        unlock_sentence(file, sent_num, msg.username);
                        for (int i = 0; i < num_words; i++) free(words[i]);
                        free(words);
                    ...
                    if (k == 0) pos = 0; else pos = (int)k; // insert before element at index 'pos'
                    if (pos > num_words) {
                        free(updates_dup);
                        unlock_sentence(file, sent_num, msg.username);
                        for (int i = 0; i < num_words; i++) free(words[i]);
                ...
                if (ins_dup) free(ins_dup);

                if (add_count > 0) {
                    // Ensure capacity in main words array
                    if (num_words + add_count > words_cap) {
                        while (num_words + add_count > words_cap) words_cap *= 2;
                        char **nw = realloc(words, words_cap * sizeof(char*));
                        if (!nw) {
                            for (int i = 0; i < add_count; i++) free(add_words[i]);
                            free(add_words);
                            free(updates_dup);
                            unlock_sentence(file, sent_num, msg.username);
                            for (int i = 0; i < num_words; i++) free(words[i]);
            ...
            size_t new_len = 0;
            for (int i = 0; i < num_words; i++) new_len += strlen(words[i]) + 1;
            ...
            if (need_space) new_content[cursor++] = ' ';
            strcpy(new_content + cursor, rebuilt);
            if (suffix_len > 0) memcpy(new_content + cursor + strlen(rebuilt), content + suffix_start, suffix_len);
            ...
            int total_sentences = 0; for (size_t i = 0; file->content[i]; i++) if (is_sentence_delimiter(file->content[i])) total_sentences++;
        ...
        case MSG_UNDO: {
            ...
            if (file->is_modified) {
                ...
                while (save_retries < max_save_retries) {
                    if (save_file(file) == 0) break;  // Success
                    save_retries++;
        ...
        case MSG_SS_INFO: {
            ...
            if (!file) {
                ...
                pthread_mutex_lock(&file->lock);
                // Recalculate counts defensively
                count_words_and_chars(file->content, &file->word_count, &file->char_count);
                long size = (long)strlen(file->content);
                time_t last_acc = file->last_access;
                time_t last_mod = file->last_modified;
                time_t created = file->created_time ? file->created_time : last_mod;
                char last_reader[MAX_USERNAME];
                strncpy(last_reader, file->last_reader, MAX_USERNAME - 1);
                last_reader[MAX_USERNAME - 1] = '\0';
                pthread_mutex_unlock(&file->lock);
                // Encode as space-separated numbers for NM
...
int main(int argc, char *argv[]) {
    ...
    if (nm_sock2 >= 0) {
        struct sockaddr_in nm_addr2; memset(&nm_addr2, 0, sizeof(nm_addr2));
        nm_addr2.sin_family = AF_INET; nm_addr2.sin_port = htons(nm_port);
        inet_pton(AF_INET, nm_ip, &nm_addr2.sin_addr);
        if (connect(nm_sock2, (struct sockaddr*)&nm_addr2, sizeof(nm_addr2)) == 0) {
            Message list_msg; memset(&list_msg, 0, sizeof(list_msg));
            ...
            Message ack2; receive_message(nm_sock2, &ack2);
        }
        close(nm_sock2);
    }
    ...
    while (1) {
        ...
        *client_sock = accept(server_sock, (struct sockaddr*)&client_addr, &client_len);
        ...
        if (*client_sock < 0) {
            if (errno == EINTR) { free(client_sock); continue; }
            if (errno == EAGAIN || errno == EWOULDBLOCK) { free(client_sock); usleep(10000); continue; }
            perror("accept");
            free(client_sock);
            continue;
        }
